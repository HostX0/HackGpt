...
class Ollama(LocalRuntime):
    ...
    def context(report):
        ...
        return {
            ...
            "checks": [
                {"tool": c["tool"], "status": c["status"], "result": c.get("result")}
    ...
    def plan(self, execute, report, checkpoint):
        ...
        tool = {
            ...
            "function": {
                ...
                "description": "Run the already-approved, non-destructive synthetic authorization proof in an owned ephemeral lab. No external target. No arguments. May execute once.",
                "parameters": {
                    "type": "object",
                    "properties": {},
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": "Choose whether to run the approved lab verification. You may stop without a proof; this is inconclusive. Context: "
                + json.dumps(self.context(report)),
            },
        ]
        decisions = []
    ...
    def summarize(self, report):
        ...
        schema = {
            ...
            "required": ["summary", "next_steps", "limitations"],
            "additionalProperties": False,
        }
        message = self.chat(
            [
                ...
                {
                    "role": "user",
                    "content": "Interpret these recorded checks without changing their verdicts. Return the requested JSON schema.\n"
                    + json.dumps(self.context(report)),
                },
            ],
            format=schema,
        ...
        for key in ("next_steps", "limitations"):
            if (
                ...
                or len(value[key]) > 12
                or any(not isinstance(x, str) or len(x) > 2000 for x in value[key])
            ):
