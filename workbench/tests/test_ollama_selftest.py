...
class SelfTestTests(unittest.TestCase):
    ...
    def test_structured_probe_marks_only_compatibility(self):
        client = self.client(
            [
                {
                    "content": json.dumps(
                        {"status": "ready", "scope": "synthetic_self_test"}
                    )
                }
            ]
        )
        ...
        client = self.client(
            [
                {"content": '{"status":"ready","scope":"synthetic_self_test"}'},
                {
                    "content": "",
                    "tool_calls": [
                        {"function": {"name": "workbench_self_test", "arguments": {}}}
                    ],
                },
            ]
        )
    ...
    def test_invalid_structured_output_fails_closed(self):
        for content in (
            "not-json",
            '{"status":"ready"}',
            '{"status":"safe","scope":"synthetic_self_test"}',
            "[]",
        ):
    ...
    def test_ollama_method_exposes_probe(self):
        ...
        client.chat = Mock(
            return_value={"content": '{"status":"ready","scope":"synthetic_self_test"}'}
        )
    ...
    def test_metadata_failure_prevents_prompt(self):
        client = self.client([])
        client.inspect_model.side_effect = OllamaError(
            "cloud_model_blocked", "blocked", "choose local"
        )
        with self.assertRaises(OllamaError) as context:
            validate_local_inference(client)
