...
class ServerTests(unittest.TestCase):
    ...
    def test_wrong_token_rejected(self):
        self.assertEqual(
            self.call("/api/health", extra={"Authorization": "Bearer wrong"})[0], 401
        )

    def test_host_rebinding_rejected(self):
        self.assertEqual(
            self.call("/api/health", extra={"Host": "attacker.example"})[0], 403
        )

    def test_cross_origin_rejected(self):
        self.assertEqual(
            self.call("/api/health", extra={"Origin": "https://elsewhere.example"})[0],
            403,
        )
    ...
    def test_content_type_rejected(self):
        self.assertEqual(
            self.call("/api/runs", INPUT, extra={"Content-Type": "text/plain"})[0], 400
        )

    def test_oversized_body_rejected_before_read(self):
        self.assertEqual(
            self.call("/api/runs", INPUT, extra={"Content-Length": "20000"})[0], 400
        )

    def test_encoding_rejected(self):
        self.assertEqual(
            self.call("/api/runs", INPUT, extra={"Content-Encoding": "gzip"})[0], 400
        )
    ...
    def test_assessment_lifecycle_history_and_exports(self):
        ...
        self.assertIn(
            b"Coverage and execution",
            self.call("/api/runs/" + run_id + "/export.md")[2],
        )
...
class StoreTests(unittest.TestCase):
    def setUp(self):
        ...
        scope = Scope.parse(
            {**INPUT, "mode": "analyst", "target": "https://example.com"}
        )
