...
class ReviewDocumentationTests(unittest.TestCase):
    ...
    def test_current_readme_has_accurate_entrypoint_and_execution_limits(self):
        ...
        for expected in ('git clone --branch main', 'python -m workbench', 'Semgrep CE 1.177.0',
                         'parser-only', 'ZAP and Nmap', 'not encrypted at rest',
                         'not a universally sanitized', 'No findings is not a security guarantee'):
            with self.subTest(expected=expected):
                self.assertIn(expected, readme)
        self.assertNotIn('Live local/cloud model inference has not yet been validated', readme)
        self.assertNotIn('executable ZAP, Nuclei, Semgrep, Trivy or Nmap runners;', readme)

    def test_model_compatibility_is_not_a_quality_or_egress_claim(self):
        doc = text('OLLAMA.md')
        for expected in ('real local-model compatibility probe', '35589788372',
                         'not analysis quality', '**not** an egress guarantee',
                         'no target, credentials or assessment evidence',
                         'shared monotonic assessment deadline'):
            with self.subTest(expected=expected):
                self.assertIn(expected, doc)
        self.assertNotIn('Socket timeouts remain per blocking operation, not a hard overall deadline', doc)
    ...
    def test_active_milestone_does_not_reuse_historical_stop_condition(self):
        ...
        self.assertNotIn('**Current bounded milestone state: Gates A, B, C, D and E pass', roadmap)

    def test_first_milestone_records_preserved_byte_for_byte(self):
        for path, expected in (('PROGRESS_HISTORY.md', '1e3f288fa2666feac06a87558f6e01e74e8d8d9d'),
    ...
    def test_current_doc_links_resolve_inside_portable_component(self):
        for name in ('README.md', 'ROADMAP.md', 'PROGRESS.md', 'OLLAMA.md', 'RELEASE_NOTES.md'):
    ...
    @unittest.skipUnless(WORKFLOWS.is_dir(), 'portable source package does not ship repository CI workflows')
