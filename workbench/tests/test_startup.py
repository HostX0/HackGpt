...
class StartupTests(unittest.TestCase):
    ...
    def check(self, *args):
        return subprocess.run(command(self.directory, '--check-install', '--json',
                                     '--port', str(unused_port()), *args),
    ...
    def test_preflight_never_connects_to_a_model_or_remote_host(self):
        with patch('socket.socket.connect', side_effect=AssertionError('no connect allowed')):
    ...
    def test_invalid_port_fails_before_creating_workspace(self):
        ...
        result = subprocess.run(command(missing, '--check-install', '--port', '80'),
    ...
    def test_ambiguous_cli_options_rejected(self):
        for args in (('--json',), ('--check-install', '--open-browser')):
            with self.subTest(args=args):
                result = subprocess.run(command(self.directory, *args), capture_output=True, text=True, timeout=10)
                self.assertEqual(result.returncode, 2)

    def test_unsupported_python_has_actionable_error(self):
        with patch.object(start.sys, 'version_info', (3, 10, 0)), contextlib.redirect_stderr(io.StringIO()) as error:
    ...
    def test_busy_port_does_not_recover_existing_running_checkpoint(self):
        ...
        report = Assessment(Scope.parse({'target': 'lab', 'mode': 'analyst',
                                         'authorized': True, 'authorization': 'Owned startup fixture'})).report
        store.save_active(report)
        with socket.socket() as listener:
            listener.bind(('127.0.0.1', 0))
            listener.listen(1)
            result = subprocess.run(command(self.directory, '--port', str(listener.getsockname()[1])),
                                    capture_output=True, text=True, timeout=10)
        self.assertEqual(result.returncode, 1)
        with contextlib.closing(sqlite3.connect(store.path)) as connection:
            self.assertEqual(connection.execute('SELECT COUNT(*) FROM active_runs').fetchone()[0], 1)
    ...
    def test_storage_failure_is_nonzero_and_does_not_expose_error_text(self):
        with patch.object(start, 'check_local_install', side_effect=OSError('sensitive-path-detail')), contextlib.redirect_stdout(io.StringIO()) as output:
            status = start.main(['--check-install', '--json', '--data-dir', str(self.directory)])
    ...
    def test_separate_workspaces_do_not_block_each_other(self):
        ...
        try:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(start.main(['--check-install', '--data-dir', str(other),
                                             '--port', str(unused_port())]), 0)
        finally:
            lock.close()

    @unittest.skipIf(os.name == 'nt', 'Windows symlink creation may require developer mode or administrator privileges')
    ...
    def test_browser_is_explicit_and_failure_does_not_claim_e2e(self):
        with patch('webbrowser.open_new_tab', return_value=False) as browser, contextlib.redirect_stdout(io.StringIO()) as output:
            self.assertEqual(start.main(['--check-install', '--data-dir', str(self.directory),
                                         '--port', str(unused_port())]), 0)
    ...
    def test_platform_helper_can_check_from_another_directory(self):
        args = ['--check-install', '--json', '--port', str(unused_port()), '--data-dir', str(self.directory)]
        if os.name == 'nt':
            call = ['cmd', '/c', str(ROOT / 'workbench' / 'start.cmd'), *args]
        else:
            call = ['sh', str(ROOT / 'workbench' / 'start.command'), *args]
        result = subprocess.run(call, cwd=self.directory, capture_output=True, text=True, timeout=10)
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_real_launcher_owned_assessment_and_duplicate_start_preserve_history(self):
        port = unused_port()
        process = subprocess.Popen(command(self.directory, '--port', str(port)),
        ...
        try:
            ...
            while time.monotonic() < until:
                try:
                    ...
                    if process.poll() is not None:
                        self.fail('launcher exited before ready: ' + process.stderr.read())
                    continue
            ...
            status, created = call('/api/runs', {'target': 'lab', 'mode': 'verify', 'authorized': True,
                                               'authorization': 'Owned startup integration fixture',
                                               'approve_verification': True, 'use_ai': False})
            ...
            active = Assessment(Scope.parse({'target': 'lab', 'mode': 'analyst',
                                             'authorized': True, 'authorization': 'Owned in-flight fixture'})).report
            Store(self.directory).save_active(active)
            second = subprocess.run(command(self.directory, '--port', str(unused_port())),
                                    capture_output=True, text=True, timeout=10)
            ...
            with contextlib.closing(sqlite3.connect(self.directory / 'reports.sqlite3')) as database:
                saved = database.execute('SELECT content FROM active_runs WHERE id = ?', (active['id'],)).fetchone()
