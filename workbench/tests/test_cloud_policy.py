...
CLOUD = "fixture:cloud"
SUMMARY = {"summary": "Observed checks only", "next_steps": [], "limitations": ["Fixture data"]}
...
class CloudProtocolTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server.responses["/api/tags"] = (200, {"models": [{"name": CLOUD}, {"name": protocol.MODEL}]})
        self.server.responses["/api/show"] = (200, {"remote_host": "https://cloud.example.invalid", "remote_model": "fixture", "capabilities": ["completion", "tools"]})
        self.server.responses["/api/chat"] = (200, {"done": True, "remote_host": "https://cloud.example.invalid", "message": {"role": "assistant", "content": json.dumps(SUMMARY)}})
    ...
    def test_cloud_summary_uses_prompted_contract_with_same_validation(self):
        ...
        self.assertEqual(self.client.telemetry()["calls"][0]["schema_strategy"], "prompt_then_validate")
    ...
    def test_invalid_cloud_json_is_not_accepted_as_summary(self):
        self.server.responses["/api/chat"] = (200, {"done": True, "message": {"content": "```json\n{}\n```"}})
        with self.assertRaises(ValueError):
            self.client.summarize(protocol.REPORT)

    def test_extra_summary_fields_are_rejected(self):
        self.server.responses["/api/chat"] = (200, {"done": True, "message": {"content": json.dumps({**SUMMARY, "verified": True})}})
        with self.assertRaises(ValueError):
            self.client.summarize(protocol.REPORT)

    def test_cloud_metadata_does_not_grant_missing_tool_capability(self):
        self.server.responses["/api/show"] = (200, {"remote_host": "remote", "capabilities": ["completion"]})
        self.assert_code("tools_unsupported", lambda: self.client.chat(protocol.MESSAGES, tools=[protocol.TOOL]))
        self.assertNotIn("/api/chat", [x[0] for x in self.server.requests])

    def test_metadata_alias_without_cloud_in_its_name_can_be_approved(self):
        self.client.model = protocol.MODEL
        self.assertEqual(self.client.inspect_model()["execution_location"], "cloud_reported")
        self.assertEqual(self.client.summarize(protocol.REPORT), SUMMARY)

    def test_catalog_remote_alias_is_not_misclassified_when_show_omits_remote_fields(self):
        self.client.model = protocol.MODEL
        self.server.responses["/api/tags"] = (200, {"models": [{"name": protocol.MODEL, "remote_model": "remote"}]})
    ...
    def test_opt_in_does_not_force_local_model_into_cloud(self):
        self.client.model = protocol.MODEL
        self.server.responses["/api/show"] = (200, {"capabilities": ["completion"]})
        self.server.responses["/api/chat"] = (200, {"done": True, "message": {"content": json.dumps(SUMMARY)}})
        self.client.summarize(protocol.REPORT)
        self.assertIn("format", self.server.requests[-1][1])
        self.assertEqual(self.client.telemetry()["execution_location"], "local_reported")

    def test_cloud_output_has_same_truncation_gate(self):
        self.server.responses["/api/chat"] = (200, {"done": True, "done_reason": "length", "message": {"content": "partial"}})
        self.assert_code("output_truncated", lambda: self.client.chat(protocol.MESSAGES))

    def test_cloud_tools_still_accept_at_most_one_call(self):
        self.server.responses["/api/chat"] = (200, {"done": True, "message": {"content": "", "tool_calls": [{"function": {}}, {"function": {}}]}})
        self.assert_code("invalid_tool_call", lambda: self.client.chat(protocol.MESSAGES, tools=[protocol.TOOL]))
    ...
    def test_cloud_quota_error_does_not_retry_or_fallback(self):
        ...
        self.assertEqual(len([r for r in self.server.requests if r[0] == "/api/chat"]), 1)
        self.assertEqual(self.client.model, CLOUD)
    ...
    def test_usage_is_reported_not_invented_or_billed(self):
        self.server.responses["/api/chat"] = (200, {"done": True, "prompt_eval_count": 41, "eval_count": 8, "message": {"content": "{}"}})
    ...
    def test_missing_or_invalid_usage_is_unknown_not_zero(self):
        for value in (None, True, -1, "100", 1.5, 10**15):
            with self.subTest(value=value):
                ...
                self.server.responses["/api/chat"] = (200, {"done": True, "prompt_eval_count": value, "eval_count": value, "message": {"content": "{}"}})
    ...
    def test_partial_usage_coverage_is_not_reported_as_a_complete_total(self):
        self.server.responses["/api/chat"] = (200, {"done": True, "prompt_eval_count": 8, "message": {"content": "{}"}})
        self.client.chat(protocol.MESSAGES)
        self.server.responses["/api/chat"] = (200, {"done": True, "message": {"content": "{}"}})
        self.client.chat(protocol.MESSAGES)
        self.assertIsNone(self.client.telemetry()["prompt_tokens_reported"])

    def test_oversized_cloud_prompt_is_blocked_before_dispatch(self):
        self.assert_code("request_too_large", lambda: self.client.chat([{"role": "user", "content": "x" * 70000}], format={"type": "object"}))
    ...
    def test_cloud_self_test_does_not_receive_assessment_data(self):
        self.server.responses["/api/chat"] = (200, {"done": True, "message": {"content": '{"status":"ready","scope":"synthetic_self_test"}'}})
    ...
    def test_complete_http_api_gateway_and_sealed_report_without_live_inference(self):
        ...
        self.server.responses["/api/chat"] = (200, {
            "done": True, "prompt_eval_count": 73, "eval_count": 11,
            "remote_model": "synthetic-cloud", "message": {"content": json.dumps(SUMMARY)}})
        with tempfile.TemporaryDirectory() as directory:
            ...
            def request(path, body=None):
                ...
                try:
                    ...
                    connection.close()
            try:
                with patch.dict(os.environ, {"HACKGPT_OLLAMA_PORT": str(self.server.server_port)}):
                    status, response = request("/api/runs", {
                        "target": "lab", "mode": "analyst", "authorized": True,
                        "authorization": "AUTH-SECRET-NOT-FOR-MODEL", "model": CLOUD,
                        "use_ai": True, "allow_cloud": True})
                    self.assertEqual(status, 202)
                    state.worker.join(timeout=5)
                    self.assertFalse(state.worker.is_alive())
                    status, report = request("/api/runs/" + response["id"] + "/export.json")
                    ...
                    self.assertEqual(report["ai"]["usage"]["prompt_tokens_reported"], 73)
                    self.assertEqual(report["ai"]["usage"]["execution_location"], "cloud_reported")
                    self.assertEqual(report["ai"]["interpretation"], SUMMARY)
                    self.assertNotIn("AUTH-SECRET-NOT-FOR-MODEL", json.dumps(self.server.requests))
...
class CloudPolicyTests(unittest.TestCase):
    def data(self, **changes):
        return {"target": "lab", "authorized": True, "authorization": "Local fixture", "model": CLOUD, "use_ai": True, "allow_cloud": True, **changes}
    ...
    def test_scope_cloud_opt_in_and_default(self):
        ...
        self.assertFalse(Scope.parse(self.data(model=protocol.MODEL, allow_cloud=False)).allow_cloud)
        with self.assertRaises(ValueError):
    ...
    def test_cloud_opt_in_does_not_permit_new_actions(self):
        assessment = Assessment(Scope.parse(self.data(mode="verify", approve_verification=True)))
        with self.assertRaises(ValueError):
            assessment.execute_action("run_shell", {}, None)
        self.assertEqual(assessment.requests_used, 0)

    def test_report_records_policy_and_keeps_missing_external_verification_visible(self):
        ai = Mock()
        ai.summarize.return_value = SUMMARY
        report = Assessment(Scope.parse(self.data(target="https://example.invalid", mode="verify", approve_verification=True)), remote_reader=lambda _: {"status": 200, "headers": {}}, ai_client=ai).run()
        self.assertTrue(report["ai"]["cloud_processing_approved"])
    ...
    def test_data_context_drops_target_authorization_evidence_and_raw_headers(self):
        report = {**protocol.REPORT, "target": "TARGET-SECRET", "authorization": "AUTH-SECRET", "findings": [{"id": "x", "rule": "header/csp", "severity": "low", "verification": "observed_only", "remediation": "Review policy", "evidence": {"body": "BODY-SECRET"}}], "checks": [{"tool": "http_baseline", "status": "completed", "evidence": {"headers": {"x-secret": "HEADER-SECRET"}}}]}
...
class CloudApiTests(unittest.TestCase):
    ...
    def test_metadata_forwards_policy_without_starting_run(self, factory):
        ...
        self.assertEqual(self.call("/api/models/check", {"model": CLOUD, "allow_cloud": True, "require_tools": True})[0], 200)
    ...
    def test_discovery_accepts_only_boolean_policy(self, factory):
        ...
        self.assertEqual(self.call("/api/models/discover", {"allow_cloud": True})[0], 200)
        factory.assert_called_once_with("", allow_cloud=True)
        factory.reset_mock()
        for body in ({"allow_cloud": "true"}, {"allow_cloud": True, "target": "https://example.invalid"}, []):
            self.assertEqual(self.call("/api/models/discover", body)[0], 400)
        factory.assert_not_called()

    @patch("workbench.server.Ollama")
    def test_cloud_diagnostics_and_selftest_still_require_api_authentication(self, factory):
        for route in ("/api/models/discover", "/api/models/check", "/api/models/self-test"):
            self.assertEqual(self.call(route, {"allow_cloud": True, "model": CLOUD}, auth=False)[0], 401)
        factory.assert_not_called()
    ...
    def test_selftest_uses_opt_in_without_assessment_fields(self, factory):
        ...
        status, _, raw = self.call("/api/models/self-test", {"model": CLOUD, "allow_cloud": True})
        self.assertEqual(status, 200)
    ...
    def test_invalid_cloud_flags_are_rejected_before_gateway_access(self, factory):
        for route in ("/api/models/check", "/api/models/self-test"):
            self.assertEqual(self.call(route, {"model": CLOUD, "allow_cloud": "yes"})[0], 400)
        factory.assert_not_called()
    ...
    def test_health_distinguishes_local_ui_from_inference_policy(self):
        ...
        self.assertEqual(health["ai_processing_policies"], ["local_only", "cloud_allowed"])
