...
class PublicScopePolicyTests(unittest.TestCase):
    ...
    def test_special_and_transition_ranges_are_denied(self):
        for address in ('0.1.2.3', '10.0.0.1', '100.64.0.1', '127.0.0.1',
                        '169.254.169.254', '172.16.0.1', '192.168.0.1',
                        '192.0.0.8', '192.0.0.9', '192.0.0.10', '192.88.99.2',
                        '192.0.2.1', '198.18.0.1', '198.51.100.1', '203.0.113.1',
                        '240.0.0.1', '255.255.255.255', '::', '::1',
                        '::ffff:8.8.8.8', '64:ff9b::808:808', '64:ff9b:1::1',
                        '100::1', '2001::1', '2001:1::1', '2001:db8::1',
                        '2002:808:808::1', '3fff::1', '5f00::1', 'fc00::1', 'fe80::1'):
            with self.subTest(address=address), self.assertRaises(ValueError):
                public_addresses('fixture.invalid', 443, resolver_for(address))

    def test_zone_identifier_is_rejected_even_on_global_address(self):
        with self.assertRaises(ValueError):
            public_addresses('fixture.invalid', 443, resolver_for('2606:4700:4700::1111%scope'))

    def test_mixed_answers_fail_before_transport_connection(self):
        for denied in ('224.0.0.1', 'fec0::1', '192.88.99.2'):
            with self.subTest(denied=denied), \
                 patch('workbench.engine.socket.getaddrinfo', resolver_for('8.8.8.8', denied)), \
                 patch('workbench.network_transport._connect_bounded', side_effect=AssertionError('connection must not start')) as connect:
                with self.assertRaises(ValueError):
                    network_transport.inspect_remote('https://fixture.invalid/', Deadline(1))
                connect.assert_not_called()

    def test_denied_dns_is_durable_failure_without_success_receipt(self):
        from workbench.adapter_lifecycle import AdapterLifecycle, verify_lifecycle_record
        from workbench.registry import ExecutionRegistry
        request = {"target": "https://fixture.invalid/", "asset_key": "owned-fixture"}
        with tempfile.TemporaryDirectory() as directory:
            lifecycle = AdapterLifecycle(Path(directory) / "reports.sqlite3", ExecutionRegistry())
            plan = lifecycle.plan("native-web-headers", request)
            lifecycle.approve(plan["id"], plan["plan_sha256"])
            with patch('workbench.engine.socket.getaddrinfo', resolver_for('8.8.8.8', 'ff02::1')), \
                 patch('workbench.network_transport._connect_bounded', side_effect=AssertionError('no connection')) as connect:
                with self.assertRaises(ValueError):
    ...
    def test_public_unicast_order_and_deduplication_remain_compatible(self):
        ...
        self.assertEqual(public_addresses('fixture.invalid', 443, resolver_for(*addresses)), list(addresses[:2]))
    ...
    def test_cancel_without_explicit_deadline_bounds_caller_wait(self):
        ...
        try:
            ...
            cancel.set()
            self.assertTrue(finished.wait(0.5), 'caller still waited on resolver after cancellation')
