    ...
    def do_POST(self):
        if self.path == "/api/runs":
            with self.server.operation_gate:
                if self.server.adapter_active is not None:
                    if not self.guard(True):
                        return
                    return self.reply(409, {"error": "A reviewed adapter execution is active. Stop or finish it before starting an assessment."})
                return super().do_POST()

        adapter_route = self.path.startswith("/api/adapters/") or self.path.startswith("/api/adapter-runs/")
        ...
        try:
            ...
            if self.path == "/api/adapters/plan":
                if set(data) != {"adapter_id", "request"} or not isinstance(data.get("adapter_id"), str) or not isinstance(data.get("request"), dict):
            ...
            if action == "approve":
                ...
                record = self.server.adapter_lifecycle.approve(lifecycle_id, data["plan_sha256"])
            ...
            if action == "cancel":
                ...
                with self.server.operation_gate:
                    if self.server.adapter_active != lifecycle_id or self.server.adapter_cancel is None:
                        return self.reply(409, {"error": "This adapter execution is not active"})
                    self.server.adapter_cancel.set()
                return self.reply(202, {"status": "cancellation_requested", "id": lifecycle_id})

            if set(data) != {"adapter_id", "request"} or not isinstance(data.get("adapter_id"), str) or not isinstance(data.get("request"), dict):
                raise ValueError("Send exactly adapter_id and the same typed request used for planning")
            with self.server.operation_gate:
                if self.server.adapter_active is not None:
                    return self.reply(409, {"error": "Another reviewed adapter execution is active"})
                if self.server.state.active is not None:
                    return self.reply(409, {"error": "An assessment is active. Finish or cancel it before adapter execution."})
            ...
            self.reply(409, {"error": "Adapter execution was cancelled; no successful receipt was fabricated."})
        except TimeoutError:
            self.reply(408, {"error": "Adapter execution exceeded its reviewed time bound."})
        except PermissionError as exc:
            self.reply(403, {"error": str(exc)})
        except (ValueError, TypeError, UnicodeDecodeError) as exc:
            self.reply(400, {"error": str(exc)})
        except Exception:
            self.reply(500, {"error": "Adapter lifecycle error. No successful execution receipt was fabricated."})
...
class WorkbenchServer(LocalServer):
    ...
    def __init__(self, address, state, token):
        ...
        self.adapter_registry = ExecutionRegistry(
            RegistryPolicy(max_effect="passive", allow_filesystem=True, allow_network=True)
        )
        self.adapter_lifecycle = AdapterLifecycle(state.store.path, self.adapter_registry)
...
def main():
    ...
    print("Native checks and reviewed adapter planning are ready. External scanners are NOT bundled in this milestone.")
    ...
    if state.recovered_interruptions:
        print(f"Recovered {state.recovered_interruptions} interrupted assessment run(s); none were marked completed.")
    if server.adapter_lifecycle.recovered_interruptions:
        print(f"Recovered {server.adapter_lifecycle.recovered_interruptions} interrupted adapter execution(s); none were marked completed.")
    try:
