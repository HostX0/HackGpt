...
**Current state after Contribution 15: Gates A, B and D pass their declared bounded criteria in the dedicated workbench test boundary.** Gate C has a durable adapter lifecycle, exact request-digest approval binding, authenticated plan/approve/execute/cancel/read API routes, serialized adapter-vs-assessment execution, a GUI authority preview, restart recovery and versioned execution receipts, but still lacks reviewed pinned/licensed third-party runner packaging/integration and therefore remains incomplete. Gate E materially advanced: hosted Evidence Workbench run `35580926585` passed a real Chromium-to-loopback E2E/accessibility job and fresh-checkout smoke jobs on GitHub-hosted Ubuntu, macOS and Windows at feature head `dfa25ac302d1bfbcf12819da606b93ba0610b69e`, alongside the existing Python 3.11/3.12/3.13 native suite. Gate E still lacks a live real-model compatibility check and signed/release-grade packaging evidence; the generated review manifest also remains conservative until cross-job validation aggregation is explicitly wired. The project is therefore **not** eligible for an early “only polish remains” stop.
...
Acceptance: the bounded Chromium/Ubuntu browser-to-loopback and keyboard/accessibility criterion is now test-backed; this is not a multi-browser accessibility certification.

## 9. Reproducible packaging

The isolated workbench remains standard-library-first and separate from legacy dependencies. A fresh-checkout smoke module now starts the real `WorkbenchServer`, exercises token-authenticated health and adapter discovery, completes the owned synthetic verification, verifies report integrity/durable SQLite persistence, exports the saved report and checks history. Hosted run `35580926585` passed the same smoke on GitHub-hosted Ubuntu, macOS and Windows with Python 3.13.

Next work: produce a signed release-grade package/install path rather than treating a source checkout as an installer. Package any future reviewed third-party tools in least-privilege containers or equivalent isolated bundles with pinned versions, checksums, SBOM entries and licenses. Avoid privileged containers, host Docker socket mounts and unreviewed shell installers. Define measured offline/update bundles.

Acceptance: fresh-checkout smoke now passes on the documented hosted OS matrix; never claim a third-party tool is bundled until its image/binary is actually present and tested.

## 10. Release-quality evidence

Implemented review-evidence foundations include [THREAT_MODEL.md](THREAT_MODEL.md), [REGRESSION_LABS.md](REGRESSION_LABS.md), [RELEASE_NOTES.md](RELEASE_NOTES.md) and `release_evidence.py`. The Python 3.13 CI job generates a revision-bound machine-readable review manifest, a minimal CycloneDX SBOM scoped to the isolated shipped workbench component, a JSON/Markdown sample report produced by the owned synthetic authorization fixture, checksums and the source/test-log review bundle. Separate CI jobs now generate real Chromium loopback E2E evidence and run the source-checkout smoke matrix on Ubuntu/macOS/Windows.

The release manifest deliberately remains conservative until the CI evidence from separate jobs is aggregated into one revision-bound release attestation; a successful browser/platform job must not be silently inferred by a manifest generated independently in another matrix job.

Next work: validate a real compatible model separately from protocol doubles; aggregate revision-bound browser/platform evidence into release review artifacts without overstating scope; define signing/key management and a release-grade package; add measured performance only after reproducible benchmarks. Future executable third-party runners must add pinned versions/images, licenses, checksums and SBOM components before the release evidence may claim they ship.

Acceptance: Gate E only after every Gate E criterion passes; browser E2E and cross-platform source-checkout smoke now pass, while live real-model and signed/release-package evidence remain blockers.
