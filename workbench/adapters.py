...
def _envelope(adapter_id: str, version: str, status: str, coverage: dict[str, Any], findings: list[dict[str, Any]], error: str | None = None) -> dict[str, Any]:
...
def parse_semgrep_json(raw: str | bytes, *, version: str, asset_key: str) -> dict[str, Any]:
    ...
    scanned = document.get("paths", {}).get("scanned", []) if isinstance(document.get("paths"), dict) else []
    ...
    for item in document.get("results", []):
        ...
        findings.append({
            ...
            "external_id": _text(extra.get("fingerprint"), f"{path}:{evidence['start_line'] or 0}:{check_id}", 200),
        })

    status = "partial" if errors else "completed"
    error = f"Semgrep reported {len(errors)} error(s); coverage may be incomplete" if errors else None
    coverage = {
...
def parse_trivy_json(raw: str | bytes, *, version: str, asset_key: str) -> dict[str, Any]:
    ...
    for result in results:
        ...
        for vuln in result.get("Vulnerabilities") or []:
            ...
            vuln_id = _text(vuln.get("VulnerabilityID"), "trivy/unknown-vulnerability", 160)
            pkg = _text(vuln.get("PkgName"), "unknown-package", 240)
            evidence = {
                "target": target,
                "package": pkg,
                "installed_version": _text(vuln.get("InstalledVersion"), "unknown", 200),
            ...
            findings.append({
                ...
                "confidence": 0.5,
                "evidence": evidence,
                "remediation": ("Upgrade to a fixed version after compatibility review." if evidence["fixed_version"] != "not reported" else "Review vendor/advisory guidance and determine an appropriate patched or mitigated version."),
                "external_id": f"{vuln_id}:{pkg}:{evidence['installed_version']}"[:200],
            })
        for misconfig in result.get("Misconfigurations") or []:
            if not isinstance(misconfig, dict):
                raise ValueError("invalid Trivy misconfiguration")
            rule = _text(misconfig.get("ID") or misconfig.get("AVDID"), "trivy/unknown-misconfiguration", 160)
            ...
            findings.append({
                ...
                "confidence": 0.5,
                "evidence": evidence,
                "remediation": _text(misconfig.get("Resolution"), "Review the configuration and apply the scanner's documented remediation guidance.", 1000),
                "external_id": f"{rule}:{target}:{evidence['start_line'] or 0}"[:200],
            })
        for secret in result.get("Secrets") or []:
            ...
            findings.append({
                "rule": rule,
                "title": _text(secret.get("Title"), "Potential secret detected", 240),
    ...
    coverage = {
        ...
        "notes": ["Trivy JSON parser omits secret matches and embedded source/configuration code."],
    }
    return normalize_adapter_result(_envelope("trivy-json", version, "completed", coverage, findings), asset_key=asset_key)


def parse_nuclei_jsonl(raw: str | bytes, *, version: str, asset_key: str) -> dict[str, Any]:
    """Parse Nuclei JSONL findings while dropping requests, responses and curl commands."""
    text = _decode(raw)
    lines = [line for line in text.splitlines() if line.strip()]
    if not lines:
        return normalize_adapter_result(_envelope(
            "nuclei-jsonl", version, "completed",
            {"objects_tested": None, "objects_total": None, "notes": ["Empty finding stream; Nuclei JSONL does not by itself prove target/template coverage."]},
            []), asset_key=asset_key)
    ...
    for line in lines:
        ...
        findings.append({
            ...
            "confidence": 0.5,
            "evidence": evidence,
            "remediation": _text(info.get("remediation"), "Review the template finding, confirm it in the approved scope, and apply product-specific remediation.", 1000),
            "external_id": _text(item.get("template-url"), f"{template_id}:{path or '/'}", 200),
        })

    coverage = {
        "objects_tested": None,
        "objects_total": None,
        "notes": ["Nuclei finding JSONL does not encode complete target/template coverage; raw requests, responses, curl commands and extracted values are omitted."],
    }
    return normalize_adapter_result(_envelope("nuclei-jsonl", version, "partial", coverage, findings), asset_key=asset_key)
...
def parse_scanner_output(adapter_id: str, raw: str | bytes, *, version: str, asset_key: str) -> dict[str, Any]:
