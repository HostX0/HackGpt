...
_ALLOWED_STATES = {"planned", "approved", "executing", "completed", "failed", "cancelled", "interrupted"}
...
def _plan_digest(adapter_id: str, declaration: dict[str, Any], request_summary: dict[str, Any], request_sha256: str) -> str:
    return _digest({
...
def _seal(record: dict[str, Any]) -> dict[str, Any]:
    ...
    raw = json.dumps(sealed, sort_keys=True, separators=(",", ":"), ensure_ascii=True).encode()
...
class AdapterLifecycleStore:
    ...
    def create(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        with closing(sqlite3.connect(self.path, timeout=5)) as connection:
            ...
            connection.execute(
                ...
                (sealed["id"], sealed["created_at"], sealed["updated_at"], sealed["status"], raw),
            )
            connection.commit()
        return sealed
    ...
    def get(self, lifecycle_id: str) -> dict[str, Any] | None:
        with closing(sqlite3.connect(self.path, timeout=5)) as connection:
            row = connection.execute("SELECT content FROM adapter_lifecycle WHERE id = ?", (lifecycle_id,)).fetchone()
        if not row:
            return None
    ...
    def replace(self, lifecycle_id: str, expected_status: str, record: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(record, dict) or record.get("id") != lifecycle_id:
            raise ValueError("adapter lifecycle identity cannot change")
        with closing(sqlite3.connect(self.path, timeout=5)) as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute("SELECT status, content FROM adapter_lifecycle WHERE id = ?", (lifecycle_id,)).fetchone()
            if not row:
            ...
            if row[0] != expected_status:
                raise ValueError(f"adapter lifecycle is {row[0]}, expected {expected_status}")
            immutable = ("schema", "id", "adapter_id", "created_at", "declaration", "request_summary", "request_sha256", "plan_sha256")
    ...
    def recover_interrupted(self) -> int:
        ...
        with closing(sqlite3.connect(self.path, timeout=5)) as connection:
            connection.execute("BEGIN IMMEDIATE")
            rows = connection.execute("SELECT id, content FROM adapter_lifecycle WHERE status = 'executing'").fetchall()
    ...
    def recent(self, limit: int = 50) -> list[dict[str, Any]]:
        ...
        with closing(sqlite3.connect(self.path, timeout=5)) as connection:
            rows = connection.execute(
                "SELECT content FROM adapter_lifecycle ORDER BY created DESC LIMIT ?", (limit,)
            ).fetchall()
        result = []
        for (raw,) in rows:
            record = json.loads(raw)
...
class AdapterLifecycle:
    ...
    def plan(self, adapter_id: str, request: dict[str, Any]) -> dict[str, Any]:
        ...
        record = {
            ...
            "request_sha256": request_sha256,
            "plan_sha256": _plan_digest(adapter_id, plan["declaration"], plan["request_summary"], request_sha256),
            "receipt": None,
            "outcome": None,
        }
    ...
    def approve(self, lifecycle_id: str, plan_sha256: str) -> dict[str, Any]:
        ...
        if record is None:
            raise KeyError("adapter lifecycle record not found")
        if not isinstance(plan_sha256, str) or not secrets.compare_digest(record["plan_sha256"], plan_sha256):
    ...
    def execute(self, lifecycle_id: str, adapter_id: str, request: dict[str, Any], *, cancel=None, web_reader=None) -> dict[str, Any]:
        record = self.store.get(lifecycle_id)
        if record is None:
            raise KeyError("adapter lifecycle record not found")
        ...
        plan = self.registry.plan(adapter_id, request)
        request_sha256 = request_digest(request)
        candidate_sha256 = _plan_digest(adapter_id, plan["declaration"], plan["request_summary"], request_sha256)
        ...
        try:
            receipt = normalize_execution_receipt(
                self.registry.execute_with_receipt(adapter_id, request, cancel=cancel, web_reader=web_reader)
            )
            if receipt["declaration"] != record["declaration"] or receipt["request_summary"] != record["request_summary"]:
            ...
            terminal["status"] = "cancelled" if isinstance(exc, InterruptedError) else "failed"
