...
def build_bundle(report: dict[str, Any], markdown_text: str, retest_diff: dict[str, Any] | None = None) -> bytes:
    ...
    if retest_diff is not None:
        files["retest-diff.json"] = json.dumps(retest_diff, indent=2, sort_keys=True).encode()
    ...
    manifest = {
        ...
        "signed": False,
        "files": {name: {"sha256": _sha(raw), "bytes": len(raw)} for name, raw in sorted(files.items())},
