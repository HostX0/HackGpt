...
def check_local_install(directory, port):
    ...
    return {
        'schema': 'hackgpt.startup-check/v1', 'status': 'passed',
        'python': sys.version.split()[0],
        'checks': ['application_files', 'sqlite_memory', 'workspace_write', 'loopback_port'],
        'assessment_database_opened': False, 'inference_started': False,
        'scanner_started': False, 'workspace_lock_scope': 'this_launcher_only',
    }
...
def open_local_browser(url):
    ...
    try:
        ...
        opened = False
    if not opened:
        print('Browser did not open. Use the private launch URL printed above.', flush=True)


def main(argv=None):
    parser = argparse.ArgumentParser(description='Start the existing local Evidence Workbench without the legacy installer.')
    parser.add_argument('--port', type=int, default=8765)
    parser.add_argument('--data-dir', type=Path, default=Path.home() / '.hackgpt-workbench')
    parser.add_argument('--check-install', action='store_true', help='Check local prerequisites without opening or recovering reports')
    parser.add_argument('--json', action='store_true', help='Machine-readable startup check result; requires --check-install')
    parser.add_argument('--open-browser', action='store_true', help='Open the private local UI; does not start AI or a scan')
    ...
    try:
        ...
        if args.check_install:
            print(json.dumps(result, sort_keys=True) if args.json else
                  'Local startup checks passed. No reports were opened, no AI/scanner was contacted. This is not full application or model validation.')
            return 0
        ...
        server = WorkbenchServer(('127.0.0.1', args.port), state, secrets.token_urlsafe(32))
        ...
        print('Keep this URL private. No tunnel/public binding. Stop with Ctrl+C.', flush=True)
        print('AI is optional: choose a model explicitly in the UI. No automatic downloads, inference or scans.', flush=True)
        print('Use this launcher for every process sharing this workspace; older entry points do not acquire its lock.', flush=True)
        ...
        if args.json:
            print(json.dumps({'schema': 'hackgpt.startup-check/v1', 'status': 'failed', 'code': code}))
        elif code == 'workspace_busy':
            print('Workspace is already in use or cannot be locked. Close its other process or choose another --data-dir. Do not delete the lock file.', file=sys.stderr)
        else:
            print('Startup failed. Check Python/SQLite, extract all application files, verify workspace permissions/free space, and choose an unused --port. No automatic repair was performed.', file=sys.stderr)
        return 1
    finally:
