...
def summarize_records(records: Any) -> dict[str, Any]:
    ...
    if not isinstance(records, list) or len(records) > 1000 or any(not isinstance(row, dict) for row in records):
    ...
    columns = sorted({str(key) for row in records for key in row if isinstance(key, str) and len(key) <= 160})
    types = {column: sorted({_type_name(row[column]) for row in records if column in row}) for column in columns}
    canaries = []
    for row_index, row in enumerate(records):
        for column, value in row.items():
            if isinstance(column, str) and isinstance(value, str) and value.startswith(_SYNTHETIC_PREFIX):
                canaries.append({"row_index": row_index, "column": column, "value": value[:160]})
                if len(canaries) >= 8:
                    break
        if len(canaries) >= 8:
            break
