...
class DatabaseManager:
    ...
    def get_pentest_session(self, session_id: str) -> Optional[PentestSession]:
        ...
        with self.get_session() as session:
            return (
                session.query(PentestSession)
                .filter(PentestSession.id == session_id)
                .first()
            )
    ...
    def get_vulnerabilities_by_session(self, session_id: str) -> List[Vulnerability]:
        ...
        with self.get_session() as session:
            return (
                ...
                .filter(Vulnerability.session_id == session_id)
                .all()
            )
    ...
    def update_phase_result(
        ...
        with self.get_session() as session:
            phase_result = (
                session.query(PhaseResult).filter(PhaseResult.id == result_id).first()
            )
            if phase_result:
    ...
    def get_phase_results(self, session_id: str) -> List[PhaseResult]:
        ...
        with self.get_session() as session:
            return (
                ...
                .filter(PhaseResult.session_id == session_id)
    ...
    # User management
    def create_user(
        self, username: str, email: str, password_hash: str, role: str = "analyst"
    ) -> str:
        """Create a new user"""
        with self.get_session() as session:
            user = User(
    ...
    def get_ai_context(
        self, session_id: str, context_type: str = None
    ) -> List[AIContext]:
    ...
    def create_attack_chain(
        ...
        chain_sequence: int,
        exploit_path: List[Dict[str, Any]],
        risk_score: float,
    ...
    def get_attack_chains(self, session_id: str) -> List[AttackChain]:
        ...
        with self.get_session() as session:
            return (
                ...
                .filter(AttackChain.session_id == session_id)
        ...
        with self.get_session() as session:
            ...
            return {
                ...
                "high_risk_chains": len(
                    [chain for chain in attack_chains if chain.risk_score >= 8.0]
                ),
            }
    ...
    def get_historical_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            session_count = (
                session.query(PentestSession)
                .filter(PentestSession.created_at >= cutoff_date)
                .count()
            )
            ...
            return {
                ...
                "vulnerability_trends": {
                    trend.severity: trend.count for trend in vuln_trends
                },
            }
