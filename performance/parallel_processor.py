...
class Worker:
    ...
    def __init__(
        ...
        self.stats = WorkerStats(worker_id=worker_id, started_at=datetime.utcnow())
        self.running = False
    ...
    def _execute_with_timeout(
        self, func: Callable, args: tuple, kwargs: dict, timeout: int
    ) -> Any:
...
class ParallelProcessor:
    ...
    def _setup_celery(self, redis_url: str):
        ...
        self.celery_app = Celery("hackgpt-tasks", broker=redis_url, backend=redis_url)
        ...
        **kwargs,
        ...
        if self.use_celery:
            ...
            celery_task = self.celery_app.send_task(
                function_name, args=args, kwargs=kwargs, task_id=task_id
            )
    ...
    def start(self):
        ...
        self.thread_executor = ThreadPoolExecutor(
            max_workers=self.max_workers, thread_name_prefix="hackgpt-thread"
        )
        ...
        if not self.use_celery:
            ...
            for i in range(min(4, self.max_workers)):  # Start 4 dedicated workers
                ...
                worker_thread = threading.Thread(
                    target=worker.start, name=f"hackgpt-{worker_id}"
                )
        ...
        self.logger.info(
            f"Parallel processor started with {self.max_workers} thread workers and {self.max_processes} process workers"
        )
    ...
    def execute_parallel(
        ...
        actual_max_workers = max_workers or (
            self.max_processes if use_processes else self.max_workers
        )
    ...
    def execute_pipeline(
        self, pipeline: List[Dict[str, Any]], data: Any, parallel: bool = True
    ) -> Any:
...
def parallel_task(
    name: str = None, priority: int = 0, max_retries: int = 3, timeout: int = None
):
